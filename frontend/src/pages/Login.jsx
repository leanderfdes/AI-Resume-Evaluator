...
export default function Login() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ msg: "Logged in successfully ✅", type: "success" });
      setTimeout(() => nav("/dashboard"), 350);
    } catch (err) {
      triggerError(err?.response?.data?.detail || "Login failed");
    } finally {
      setLoading(false);
    }
  };
    ...
    <PageFade>
      ...
      <div className="min-h-screen bg-zinc-50 flex items-center justify-center px-4">
        <div className={`w-full max-w-md rounded-2xl border bg-white p-6 shadow-sm ${shake ? "shake" : ""}`}>
          ...
          <p className="text-sm text-zinc-500 mt-1">Login to view your resume reports.</p>
